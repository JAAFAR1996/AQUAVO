...
function buildMcpServer(): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      return { content: [{ type: "text", text: `خطأ: ${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
  });
...
// Store transports per session for cleanup
...
const httpServer = http.createServer(async (req, res) => {
  ...
  // Bearer token check on all non-OPTIONS requests
  const auth = req.headers["authorization"] ?? "";
  const bearer = auth.startsWith("Bearer ") ? auth.slice(7).trim() : "";
  if (bearer !== MCP_TOKEN) {
    res.writeHead(401, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: "Unauthorized — invalid or missing AQUAVO_MCP_TOKEN" }));
    return;
  }
  ...
  if (req.url === "/mcp" || req.url === "/") {
    ...
    if (req.method === "DELETE") {
      ...
      if (t) { await t.close(); sessions.delete(sessionId); }
