...
function shell(title: string, content: string, script = ""): string {
  return `<!doctype html>
...
<body><main class="wrap"><div class="brand">AQUAVO</div>${content}</main>${script ? `<script>${script}</script>` : ""}</body>
</html>`;
}

function csrfScript(): string {
  return `function csrf(){const k='aquavo_csrf_token';let t=sessionStorage.getItem(k);if(!t){const a=new Uint8Array(32);crypto.getRandomValues(a);t=Array.from(a,b=>b.toString(16).padStart(2,'0')).join('');sessionStorage.setItem(k,t)}return t}`;
}
...
function successPage(state: VerifiedOnlinePaymentState): string {
  ...
  const script = `${csrfScript()}
  try{localStorage.removeItem('aquavo_cart-v2');sessionStorage.removeItem('aquavo_checkout_attempt_v1');sessionStorage.removeItem('aquavo_applied_coupon_v1')}catch{}
  fetch('/api/cart/',{method:'DELETE',credentials:'include',headers:{'X-CSRF-Token':csrf()}}).catch(()=>{});`;
  return shell("تم الدفع بنجاح", content, script);
}
...
function failedPage(state: VerifiedOnlinePaymentState): string {
  ...
  const script = `${csrfScript()}
  const btn=document.getElementById('retry'),err=document.getElementById('retryError');
  btn.addEventListener('click',async()=>{btn.disabled=true;btn.textContent='جارٍ تجهيز الدفع...';err.hidden=true;try{const r=await fetch('/api/payments/alqaseh/order/'+encodeURIComponent(${js(state.orderId)})+'/retry',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf()},body:JSON.stringify({paymentId:${js(state.paymentId)}})});const d=await r.json().catch(()=>({}));if(r.status===409&&d.payment){location.reload();return}if(!r.ok||!d.redirectUrl)throw new Error(d.message||'تعذر إنشاء محاولة دفع جديدة');location.assign(d.redirectUrl)}catch(e){err.textContent=e&&e.message?e.message:'تعذر إعادة المحاولة حالياً';err.hidden=false;btn.disabled=false;btn.textContent='إعادة محاولة الدفع'}});`;
  return shell("لم تكتمل عملية الدفع", content, script);
}
...
function pendingPage(state: VerifiedOnlinePaymentState): string {
  const content = `<section class="card">
    ...
    <div class="actions">
      <button class="btn primary" type="button" onclick="location.reload()">تحقق مرة أخرى</button>
      <a class="btn" href="/products">العودة للتسوق</a>
    </div>
