...
/**
 * Draft review for auto-generated articles.
 *
 *   GET   /api/blog/admin/drafts        unpublished articles, newest first
 *   PATCH /api/blog/admin/drafts/:id    { publish: true } to publish, false to unpublish
 *
 * The generator now writes `isPublished: false` (see auto-blog-generator.ts), so
 * without these an accepted draft would have no way to reach readers. Publishing
 * is the deliberate act of an admin, which is the entire point: the defects that
 * reached customers — unsafe cooling advice, dead links, a false filtration
 * premise — all shipped because generation implied publication.
 *
 * These sit behind requireAdmin, and are declared after the public routes so
 * "/admin/drafts" can never be captured by "/posts/:slug".
 */
router.get("/admin/drafts", requireAdmin, async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const db = getDb();
        if (!db) { res.status(503).json({ error: "Database service unavailable" }); return; }
        const drafts = await db.query.blogPosts.findMany({
            where: eq(blogPosts.isPublished, false),
            orderBy: desc(blogPosts.createdAt),
        });
        res.json(drafts.map((p) => ({
            id: p.id,
            title: p.title,
            slug: p.slug,
            category: p.category,
...
router.patch("/admin/drafts/:id", requireAdmin, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const db = getDb();
        if (!db) { res.status(503).json({ error: "Database service unavailable" }); return; }
        // sendBeacon and similar can leave req.body undefined.
        const { publish } = (req.body ?? {}) as { publish?: unknown };
        if (typeof publish !== "boolean") {
            res.status(400).json({ error: "publish must be a boolean" });
            return;
        }
        const [updated] = await db
            ...
            .set({
                isPublished: publish,
                // Stamp publishedAt on first publish; clear it when withdrawing so
                // publishedAt ordering never surfaces an unpublished article.
        ...
        res.json({ id: updated.id, slug: updated.slug, isPublished: updated.isPublished });
    } catch (error) {
        next(error);
    }
});
