...
import { findInternalLinkViolations } from "../../shared/internal-links.js";
import {
...
function validateGeneratedBlogData(
  ...
  // Dead-link refusal, not a style preference.
  //
  // Three published articles shipped 11 internal links to routes that do not
  // exist (/filtration, /temperature-control, /substrate, …), and a fourth
  // carried seven more before it was unpublished on 2026-09-20. None of the
  // gates above catch this: a dead link is not a false claim, a foreign script
  // or a competitor referral. Every one of them was also faithfully reproduced
  // into English and Sorani, so one generator defect became three broken pages.
  // See shared/internal-links.ts.
  const links = findInternalLinkViolations(content);
  if (links.length > 0) {
    throw new Error(`BLOG_CONTENT_DEAD_LINK:${links[0].rule}:${links[0].evidence.slice(0, 160)}`);
  }
...
async function saveBlogToDatabase(blog: GeneratedBlog): Promise<boolean> {
  ...
  try {
    await db.insert(blogPosts).values({
      ...
      // Generated articles land as DRAFTS and need an admin to publish them.
      //
      // Every gate above is a refusal for a defect class we already shipped, and
      // each was added after the fact. The pattern is the point: the generator
      // keeps finding new ways to be wrong that no existing gate anticipates —
      // unsafe cooling advice, a false "filtration cools the water" premise, ice
      // cubes into the tank, seven dead links, a title reading "a guide for
      // candidates". Those reached customers because generation implied
      // publication. Gates catch the defect classes we have already met; a human
      // catches the next one.
      //
      // publishedAt stays null until someone publishes it, so ordering by
      // publishedAt keeps drafts out of every public listing.
      isPublished: false,
      isFeatured: false,
