# Effective-Date Cost Change UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an admin-only "تغييرات الكلف" tab in the Finance Center that lets admins record product cost changes with an effective date, view the history timeline, and never touch checkout or old orders.

**Architecture:** One new component file (`finance-cost-changes.tsx`) and one line added to the finance page tabs. All API endpoints already exist on the server. No backend changes needed.

**Tech Stack:** React 19, TanStack Query v5, inline styles (dark theme matching existing finance components), `addCsrfHeader`, `useToast`, `accountingCostUpdateSchema` from `@shared/accounting`.
...
|--------|------|----------------|
| Create | `client/src/components/admin/finance-cost-changes.tsx` | Full UI: product selector, cost form, history timeline, safety warnings |
| Modify | `client/src/pages/admin/finance.tsx` | Add "تغييرات الكلف" tab + import |

## API Endpoints Used (all existing, no backend changes)

| Method | URL | Purpose |
|--------|-----|---------|
| GET | `/api/products?search=&limit=30` | Search/list products with name, slug, costPrice, packagingCost, insertCost |
| GET | `/api/admin/accounting/cost-history/:productId` | Fetch history sorted `effectiveFrom DESC, createdAt DESC` |
| POST | `/api/admin/accounting/costs/:productId` | Record cost change (atomic: updates product + inserts history row) |

## Schema Reference (from `shared/accounting.ts`)
...
### Task 1: Create `finance-cost-changes.tsx`

**Files:**
- Create: `client/src/components/admin/finance-cost-changes.tsx`
...
function emptyForm(product?: Product): FormState {
  return {
    ...
    packagingCost: product?.packagingCost ? String(Math.round(Number(product.packagingCost))) : "",
...
function validateForm(f: FormState): FormErrors {
  ...
  if (f.costPrice === "" || isNaN(cp) || cp < 0) errors.costPrice = "قيمة غير صالحة (يجب أن تكون >= 0)";
  if (f.packagingCost === "" || isNaN(pk) || pk < 0) errors.packagingCost = "قيمة غير صالحة (يجب أن تكون >= 0)";
  if (f.insertCost === "" || isNaN(ic) || ic < 0) errors.insertCost = "قيمة غير صالحة (يجب أن تكون >= 0)";
...
export function FinanceCostChanges() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Refresh the product in the list so current cost badge updates
      queryClient.invalidateQueries({ queryKey: ["products-search", search] });
    },
    onError: (e) => {
  ...
  const costMissing =
    selectedProduct &&
    (!Number(selectedProduct.costPrice) || Number(selectedProduct.costPrice) === 0);
  ...
  return (
    <div dir="rtl">
      ...
      <div style={S.warningBox}>
        ...
        <div style={{ marginTop: 2 }}>أي طلب سابق يبقى محسوب حسب السجل الفعّال بتاريخ الطلب.</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16 }}>
        ...
        <div>
          ...
          <div style={S.card}>
            ...
            <input
              style={S.input}
              placeholder="اسم المنتج أو slug..."
              value={search}
              onChange={e => setSearch(e.target.value)}
            />

            {loadingProducts && (
              <div style={{ color: "#64748b", fontSize: 12, marginTop: 10 }}>جاري التحميل...</div>
            )}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: isSelected ? "1px solid #199bb840" : "1px solid transparent",
                    }}
                  >
                    <div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: isSelected ? 700 : 400 }}>
                      {p.name}
                    </div>
            ...
            <>
              ...
              <div style={{ ...S.card, marginBottom: 12 }}>
                {costMissing && (
                  <div style={{
                    ...
                    تحذير: هذا المنتج لا يحتوي على كلفة محددة — الأرقام غير دقيقة
                  </div>
                )}
                  ...
                  {[
                    ...
                    <div key={label} style={{ textAlign: "center" }}>
                      ...
                      <div style={{ color: "#e2e8f0", fontSize: 14, fontWeight: 700 }}>
                        {Number(val) > 0 ? fmt(Number(val)) : <span style={{ color: "#ef4444" }}>—</span>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={S.card}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
                  {(["costPrice", "packagingCost", "insertCost"] as const).map((key) => {
                    ...
                    return (
                      <div key={key}>
                        ...
                        />
                        {formErrors[key] && <div style={S.errorText}>{formErrors[key]}</div>}
                      </div>
                    );
                  })}
                </div>
      ...
      {selectedProduct && (
        <div style={{ marginTop: 8 }}>
          <div style={S.sectionTitle}>سجل تغييرات الكلفة — {selectedProduct.name}</div>
          <div style={S.card}>
            {loadingHistory && (
              <div style={{ color: "#64748b", fontSize: 12 }}>جاري تحميل السجل...</div>
            )}
            {historyError && (
              <div style={{ color: "#ef4444", fontSize: 12 }}>{errMsg(historyError)}</div>
            )}
            {history && history.length === 0 && (
              <div style={{ color: "#64748b", fontSize: 12 }}>لا يوجد سجل تغييرات لهذا المنتج</div>
            )}
            {history && history.length > 0 && (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr>
                      {["تاريخ التفعيل", "كلفة المنتج", "تغليف", "إنسيرت", "ملاحظة", "وقت الإدخال"].map(h => (
                  ...
                  <tbody>
                    {history.map((row, i) => (
                      <tr key={row.id} style={{ background: i === 0 ? "#0d2a3f" : "transparent" }}>
                        <td style={S.td}>
                        ...
                        <td style={{ ...S.td, color: "#94a3b8", maxWidth: 180 }}>
                          {row.note ?? "—"}
                        </td>
...
Run: `pnpm check` — should pass with no new errors.

---

### Task 2: Add tab to `finance.tsx`
...
Add tab content after expenses content:
```tsx
...
Expected: successful build, finance chunk regenerated.

---

### Task 4: Manual test in browser

- [ ] Open `/admin/finance` → "تغييرات الكلف" tab
- [ ] Search for `yee-c1-1082-2a` → select it → confirm costPrice 3440 / packagingCost 500 / insertCost 250 shows
- [ ] Submit cost change with note "اختبار واجهة تغيير الكلفة" → success toast
- [ ] Confirm new row appears first in history table
- [ ] Revert: submit 3440 / 500 / 250 with note "إرجاع بعد اختبار الواجهة" → success toast
- [ ] Confirm revert row is now first in history
- [ ] Check accounting summary still returns `success: true`
...
```bash
git add client/src/components/admin/finance-cost-changes.tsx client/src/pages/admin/finance.tsx
git commit -m "finance: add effective-date cost change UI"
```
