...
// validation is not shown as approvable, because no endpoint would accept it.
// The only route out is «تحتاج تغليف يدوي», which is not a validated plan and
// releases every carton hold.
...
import { AlertTriangle, Box, Layers, PackageCheck } from "lucide-react";
import {
  MISSING_FIELD_LABEL,
  bpToPercent,
  formatCm,
  ...
  return (
    <div dir="rtl" className="space-y-4" data-testid="carton-plan-viewer">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* Only a passing plan can be stored, reserved or confirmed. */}
        <Button
          variant="secondary"
      ...
      {result?.outcome === "manual_review" && <ManualReview result={result} />}
