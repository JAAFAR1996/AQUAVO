-- 0085_inventory_reconciliation_owner_correction
-- Owner-confirmed pre-cutover inventory correction. Runs only after production
-- test orders have been quarantined so synthetic stock/accounting cannot distort
-- the reconciliation. No P&L impact; historical evidence is preserved.
...
BEGIN
  IF NOT EXISTS(
    ...
    RAISE EXCEPTION '0085_REQUIRES_ACTIVE_0083_TEST_ORDER_ISOLATION';
  END IF;
END
$guard$;

-- Immutable 2026-08-08 baseline is the authority for the six YEE variant costs.
DO $do$
DECLARE
  v_missing bigint;
...
BEGIN
  ...
  IF v_missing<>0 THEN
    RAISE EXCEPTION '0085_VERIFIED_YEE_COST_BASELINE_MISMATCH: %',v_missing;
  END IF;

  SELECT COUNT(*) INTO v_changed_after_baseline
  ...
  IF v_changed_after_baseline<>0 THEN
    RAISE EXCEPTION '0085_YEE_COST_CHANGED_AFTER_VERIFIED_BASELINE: %',v_changed_after_baseline;
  END IF;
END
$do$;

-- Refuse conflicts; fill only missing costPrice values.
DO $do$
DECLARE v_conflicts bigint;
BEGIN
  WITH expected(variant_id,unit_cost_iqd) AS (VALUES
    ('35cube'::text,12200::numeric),
    ('40cube'::text,16400::numeric),
    ...
    SELECT v.elem->>'id' AS variant_id,
           NULLIF(v.elem->>'costPrice','')::numeric AS cost_price
  ...
  IF v_conflicts<>0 THEN
    RAISE EXCEPTION '0085_YEE_CURRENT_COST_CONFLICT: %',v_conflicts;
  END IF;
END
$do$;
...
WITH expected AS (
  ...
  SELECT jsonb_agg(
    CASE
      WHEN e.unit_cost_iqd IS NOT NULL
       AND NULLIF(v.elem->>'costPrice','') IS NULL
...
SET variants=rebuilt.variants,updated_at=clock_timestamp()
FROM rebuilt
WHERE p.id='yee-06255';

-- Quantities are owner-confirmed and must agree with the canonical MAIN ledger.
DO $do$
DECLARE
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_variant_mismatch
  FROM expected e
  LEFT JOIN current_variants c ON c.variant_id=e.variant_id
  WHERE c.variant_id IS NULL OR c.qty<>e.qty;

  IF v_variant_mismatch<>0 THEN
    RAISE EXCEPTION '0085_OWNER_CONFIRMED_YEE_QUANTITY_MISMATCH: %',v_variant_mismatch;
  END IF;
  ...
  IF v_global_stock_mismatch<>0 THEN
    RAISE EXCEPTION '0085_CANONICAL_STOCK_MISMATCH: %',v_global_stock_mismatch;
  END IF;
END
$do$;
...
WITH current_keys AS (
  SELECT p.id product_id,NULL::text variant_id,p.stock::numeric quantity,
         p.cost_price::numeric unit_cost
  FROM public.products p
  WHERE p.deleted_at IS NULL AND COALESCE(p.has_variants,false)=false
  UNION ALL
  ...
  WHERE COALESCE(o.is_test,false)=false
    AND lower(COALESCE(o.status,'')) IN ('pending','confirmed','processing','shipped')
...
BEGIN
  ...
  IF v_batch_effect<>131480 THEN
    RAISE EXCEPTION '0085_YEE_RECONCILIATION_VALUE_MISMATCH: %',v_batch_effect;
  END IF;
  ...
  IF v_missing_current<>0 OR v_missing_unrealized<>0 THEN
    RAISE EXCEPTION
      '0085_INVENTORY_COST_EVIDENCE_INCOMPLETE: current=% unrealized=%',
      v_missing_current,v_missing_unrealized;
  END IF;
  ...
  IF NOT EXISTS(
    ...
    IF v_gap<>131480 THEN
      RAISE EXCEPTION '0085_PRE_CORRECTION_GL_GAP_MISMATCH: %',v_gap;
    END IF;
    ...
    INSERT INTO public.journal_entries(
      ...
      'inventory_reconciliation','INVENTORY-UNIFY-20260817',
      'pre_cutover_owned_inventory_valuation_correction',
      'تصحيح قيمة مخزون مملوك قبل 1 آب بعد إلغاء تصفير مخزون خاطئ',
      131480,131480,
      jsonb_build_object(
        'ownership','confirmed_pre_cutover','adjustment_iqd',131480,
        'classification','opening_equity_inventory_correction','pnl_impact_iqd',0,
    ...
    INSERT INTO public.journal_lines(
      ...
      jsonb_build_object(
        'batch','INVENTORY-UNIFY-20260817',
        'classification','pre_cutover_owned_inventory'
      )
    );
...
SELECT
  ...
  'inventory_reconciliation','INVENTORY-UNIFY-20260817','gl_inventory_valuation',
...
BEGIN
  ...
  IF v_missing_current<>0 OR v_missing_unrealized<>0 OR v_gap<>0 THEN
    RAISE EXCEPTION
      '0085_POST_CORRECTION_INVENTORY_RECONCILIATION_FAILED: current=% unrealized=% gap=%',
  ...
  IF EXISTS(
    ...
    RAISE EXCEPTION '0085_REVIEW_FLAG_STILL_OPEN';
  END IF;
END
$do$;
...
VALUES(
  '0085_inventory_reconciliation_owner_correction',
  '0085008500850085008500850085008500850085008500850085008500850085',
  'After test-order quarantine: restore six verified YEE variant costs; debit inventory 1200 / credit owner equity 3100 by 131,480 IQD; resolve the valuation review flag; P&L impact zero'
)
