...
-- `returned` has two historical uses in the UI. Only rejected -> returned means
-- physical receipt of a COD-refused parcel. delivered -> returned is a true
-- post-delivery/warranty path and must use item-level verified return events;
-- it must never restore every order line automatically.
BEGIN;

CREATE OR REPLACE FUNCTION public.reverse_order_inventory_on_terminal_status()
RETURNS trigger
LANGUAGE plpgsql
AS $inventory_workflow_0065$
DECLARE
  mode text;
  main_location text;
  item_row record;
