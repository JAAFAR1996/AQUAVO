...
-- Removes the post-delivery messaging outbox. Safe only when no unsent lifecycle
-- work remains; otherwise rollback would silently discard customer follow-ups.
...
SET rolled_back_at=clock_timestamp(),
    notes=COALESCE(notes,'') || ' [post-delivery messaging outbox rolled back after queue drain]'
