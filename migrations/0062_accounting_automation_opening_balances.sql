...
-- Complete the August cutover ledger, derive live balances, close ended Baghdad
...
BEGIN
  ...
  IF NEW.status='verified' AND OLD.status IS DISTINCT FROM 'verified' THEN
    v_cash_loss:=COALESCE(NEW.delivery_cost_loss,0)
      ...
      +CASE WHEN COALESCE(NEW.packaging_loss_source,'manual')='manual'
            THEN COALESCE(NEW.packaging_loss,0) ELSE 0 END;
    ...
    INSERT INTO public.journal_entries(
      ...
      v_total,v_total,
      jsonb_build_object(
        'order_id',NEW.order_id,'restocked',NEW.restocked,'note',NEW.note,
...
RETURNS trigger LANGUAGE plpgsql AS $line_guard$
DECLARE
  v_entry_id text;
  v_period text;
BEGIN
