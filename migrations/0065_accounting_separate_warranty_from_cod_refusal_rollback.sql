-- 0065_accounting_separate_warranty_from_cod_refusal_rollback.sql
-- Restore the broader 0063 terminal-status behavior.
BEGIN;

CREATE OR REPLACE FUNCTION public.reverse_order_inventory_on_terminal_status()
RETURNS trigger
LANGUAGE plpgsql
AS $inventory_refusal_0063_restore$
DECLARE
  mode text;
  main_location text;
  item_row record;
...
BEGIN
  ...
  END LOOP;
  RETURN NEW;
END $inventory_refusal_0063_restore$;
