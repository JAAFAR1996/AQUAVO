...
-- Repair ordering drift where a late application of 0062 can overwrite the
-- owner-approved refusal/warranty function introduced by 0065.
BEGIN;

CREATE OR REPLACE FUNCTION public.reverse_order_inventory_on_terminal_status()
RETURNS trigger
LANGUAGE plpgsql
AS $inventory_workflow_0066$
DECLARE
  mode text;
  main_location text;
  item_row record;
...
BEGIN
  ...
  END LOOP;
  RETURN NEW;
END $inventory_workflow_0066$;

INSERT INTO public.schema_migrations(version,checksum,applied_by,notes)
SELECT
  '0066_accounting_reassert_refusal_inventory_after_0062',
  '6096bd0c242ce1c2e827d9aa1802df4483370f28ffdfc105eb399d634c1d59b8',
  current_user,
  'Reassert owner-approved immediate refusal availability and item-level warranty behavior after any late 0062 application'
WHERE NOT EXISTS (
