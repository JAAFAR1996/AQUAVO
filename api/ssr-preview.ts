...
const STATIC_COPY: Record<string, { heading: string; summary: string; title?: string }> = {
  "/contact": {
    heading: "تواصل مع AQUAVO",
    summary: "بيانات التواصل والاستفسار عن الطلبات واختيار معدات أحواض الزينة في العراق.",
  },
  "/shipping": {
    heading: "الشحن والتوصيل",
    summary: "معلومات التوصيل داخل العراق، أهلية الطلب، وبيانات الشحن التي تظهر قبل تأكيد الطلب.",
  },
  "/return-policy": {
    heading: "سياسة الاسترجاع والاستبدال",
    summary: "الشروط والإجراءات المتعلقة بطلبات الاسترجاع أو الاستبدال في متجر AQUAVO.",
  },
  "/privacy-policy": {
    heading: "سياسة الخصوصية",
    summary: "كيفية جمع بيانات المستخدم واستخدامها وحمايتها عند استخدام موقع AQUAVO.",
  },
  "/terms": {
  ...
  "/why-aquavo": {
    ...
    summary: "متجر عراقي متخصص، معلومات أوضح قبل الشراء، ومنتجات موجهة لاحتياجات أحواض الزينة.",
  },
  "/beginner-guide": {
  ...
  "/fish-health": {
    ...
    summary: "معلومات أولية تساعد على ملاحظة الأعراض والمشاكل الشائعة، ولا تغني عن تشخيص مختص عند الحالات الخطرة.",
  },
  "/fish-health-diagnosis": {
    heading: "مساعد تشخيص مشاكل الأسماك",
...
function injectPreviewDocument(template: string, meta: PreviewMeta, shell: string): string {
  const canonical = `${PRODUCTION_BASE}${meta.canonicalPath}`;
  ...
  let html = template
    ...
    .replace(/__META_IMAGE__/g, escapeAttribute(image))
  ...
  if (!rootPattern.test(html)) {
    throw new Error("SSR preview root outlet was not found in the generated Vite template");
  }

  return html.replace(
    rootPattern,
    `<div id="root"$1 data-aq-server-rendered="preview">${shell}</div>`,
  );
}

function firstImage(product: SeoPreviewProduct): string | undefined {
  if (typeof product.thumbnail === "string" && product.thumbnail.length > 0) return product.thumbnail;
  if (Array.isArray(product.images)) {
    const candidate = product.images.find((value) => typeof value === "string" && value.length > 0);
...
function absoluteImage(url: string | undefined): string {
  ...
  return `${PRODUCTION_BASE}${url.startsWith("/") ? "" : "/"}${url}`;
}
...
async function loadProducts(): Promise<SeoPreviewProduct[]> {
  const db = getPool();
  if (!db) return [];
  const { rows } = await db.query(
    `SELECT id, slug, name, description, price, currency, brand, category,
...
function formatSchemaPrice(value: SeoPreviewProduct["price"]): string {
  const numeric = Number(value ?? 0);
  return Number.isFinite(numeric) && numeric >= 0 ? String(numeric) : "0";
}

function productSchema(product: SeoPreviewProduct): object[] {
  ...
  return [
    {
      ...
      image: [absoluteImage(firstImage(product))],
      ...
      offers: {
        ...
        seller: { "@type": "Organization", name: "AQUAVO", url: PRODUCTION_BASE },
      },
    },
    {
      "@context": "https://schema.org",
...
function collectionSchema(products: SeoPreviewProduct[]): object[] {
  return [
    {
      "@context": "https://schema.org",
      "@type": "CollectionPage",
...
function homeSchema(): object[] {
  return [
    ...
    },
    {
      "@context": "https://schema.org",
      "@type": "WebSite",
      "@id": `${PRODUCTION_BASE}/#website`,
      name: "AQUAVO",
      url: PRODUCTION_BASE,
      inLanguage: "ar-IQ",
      potentialAction: {
        "@type": "SearchAction",
        target: `${PRODUCTION_BASE}/products?search={search_term_string}`,
...
function renderMarkdown(page: SeoPreviewPage, meta: PreviewMeta): string {
  const lines = [`# ${meta.title}`, "", meta.description, "", `Canonical: ${PRODUCTION_BASE}${meta.canonicalPath}`, ""];
  if (page.kind === "products" || page.kind === "home") {
    const products = page.products;
    lines.push("## Products", "");
    for (const product of products) {
      lines.push(`- [${product.name}](${PRODUCTION_BASE}/products/${encodeURIComponent(product.slug)})`);
    }
  ...
  lines.push("", "Preview policy: noindex. This response is for technical validation only.");
  return lines.join("\n");
}

async function resolvePreview(pathname: string): Promise<{ page: SeoPreviewPage; meta: PreviewMeta; status: number }> {
  if (pathname === "/" || pathname === "/ar") {
    const products = await loadProducts();
    return {
      page: { kind: "home", products },
      meta: {
        title: "AQUAVO — مستلزمات أحواض الزينة في العراق",
  ...
  if (pathname === "/products") {
    ...
    return {
      ...
      meta: {
        ...
        description: "تصفح فلاتر وسخانات وأغذية وإضاءة وديكورات ومعالجات مياه ومستلزمات صيانة أحواض الزينة.",
        canonicalPath: "/products",
        jsonLd: collectionSchema(products),
      },
      status: 200,
    };
  }
  ...
  if (productMatch) {
    ...
    if (!product) {
      return {
        ...
        meta: {
          ...
          description: "المنتج المطلوب غير موجود أو لم يعد متاحاً.",
    ...
    const products = await loadProducts();
    const related = products.filter((item) => item.slug !== product.slug && item.category === product.category);
    return {
  ...
  if (copy) {
    return {
      ...
      meta: {
        title: copy.title || `${copy.heading} | AQUAVO`,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Existing guide renderer is already complete HTML. Keep it untouched.
  ...
  try {
    ...
    const { page, meta, status } = await resolvePreview(pathname);
    ...
    if (accept.includes("text/markdown")) {
      ...
      return res.status(status).send(renderMarkdown(page, meta));
    }

    const shell = renderSeoPreviewShell(page);
    const html = injectPreviewDocument(HTML_TEMPLATE, meta, shell);
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    return res.status(status).send(html);
  } catch (error) {
    console.error("[SEO Preview] SSR rendering failed", error);
    res.setHeader("X-Robots-Tag", "noindex, nofollow, noarchive");
    res.setHeader("Content-Type", "text/html; charset=utf-8");
